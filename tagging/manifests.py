...
class ManifestHeader:
    ...
    @staticmethod
    def create_header(short_image_name: str, owner: str, build_timestamp: str, registry: str) -> str:
        commit_hash = GitHelper.commit_hash()
        commit_hash_tag = GitHelper.commit_hash_tag()
        commit_message = GitHelper.commit_message()

        image_size = docker[
            "images", f"{registry}/{owner}/{short_image_name}:latest", "--format", "{{.Size}}"
        ]().rstrip()
        ...
        return "\n".join(
            [
                ...
                f"* Git commit SHA: [{commit_hash}](https://github.com/{owner}/commit/{commit_hash})",
                "* Git commit message:",
                "```",
                f"{commit_message}",
                "```",
